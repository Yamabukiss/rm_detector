...
void Detecter::initialize( const ros::NodeHandle& nh)
{
  Detecter::nh_ = ros::NodeHandle(nh, "detecter_node");
  camera_sub_=nh_.subscribe("/galaxy_camera/image_raw",20,&Detecter::receiveFromCam,this);
  camera_pub_=nh_.advertise<sensor_msgs::Image>("armor_detect",1);
//  Detecter::it_ = std::make_shared<image_transport::ImageTransport>(nh_);
...
//  camera_pub_=it_->advertise("armor_detect",1);
 ...
 void Detecter::receiveFromCam(const sensor_msgs::ImageConstPtr& image)
{

  try
  {
    Detecter::cv_image_ = boost::make_shared<cv_bridge::CvImage>(*cv_bridge::toCvShare(image, image->encoding));
    REPEAT = true;
  }
 ...
 bool Detecter::getCam(cv_bridge::CvImagePtr &cv_image, sensor_msgs::CameraInfoConstPtr &cam_info) {
  if (cv_image_ == nullptr)
    return false;
  if (!Detecter::cv_image_->image.empty() && REPEAT) {
    cv_image = Detecter::cv_image_;
    cam_info = Detecter::info_;
    REPEAT = false;
    return true;
  } else
    return false;
     ...
     void Detecter::sendMsg(boost::shared_ptr<sensor_msgs::Image> msg)
    {
...
//    }
    ...
    void Detecter::blobFromImage(cv::Mat& img, InferenceEngine::MemoryBlob::Ptr &mblob){
        int channels = 3;
        int img_h = img.rows;
        int img_w = img.cols;
        if (!mblob)
     ...
     void Detecter::generateGridsAndStride(const int target_w, const int target_h, std::vector<int>& strides, std::vector<GridAndStride>& grid_strides)
    {
     ...
     void Detecter::generateYoloxProposals(std::vector<GridAndStride> grid_strides, const float* feat_ptr, float prob_threshold, std::vector<Object>& objects)
    {
     ...
     inline float Detecter::intersectionArea(const Object& a, const Object& b)
    {
        cv::Rect_<float> inter = a.rect & b.rect;
        return inter.area();
    }

     void Detecter::qsortDescentInplace(std::vector<Object>& faceobjects, int left, int right)
    {
...
#pragma omp section
     ...
     void Detecter::qsortDescentInplace(std::vector<Object>& objects)
    {
        if (objects.empty())
            return;
     ...
     void Detecter::nmsSortedBboxes(const std::vector<Object>& faceobjects, std::vector<int>& picked, float nms_threshold)
    {
        picked.clear();
     ...
     void Detecter::decodeOutputs(const float* prob, std::vector<Object>& objects, float scale, const int img_w, const int img_h) {
        std::vector<Object> proposals;
        std::vector<int> strides = {8, 16, 32};
        std::vector<GridAndStride> grid_strides;

        generateGridsAndStride(INPUT_W, INPUT_H, strides, grid_strides);
        generateYoloxProposals(grid_strides, prob,  g_bbox_conf_thresh, proposals); // initial filtrate
...
//        objects[i] = proposals[picked[i]];
    ...
    sensor_msgs::ImagePtr Detecter::drawObjects(const cv::Mat& bgr, const std::vector<Object>& objects)
    {
...
//                    cv::FONT_HERSHEY_SIMPLEX, 0.4, txt_color, 1);
    ...
    sensor_msgs::ImagePtr Detecter::mainFuc(cv_bridge::CvImagePtr& image_ptr,std::vector<Object> objects) {

        cv::Mat pr_img = staticResize(image_ptr->image);
        blobFromImage(pr_img, Detecter::mblob_);
        Detecter::infer_request_.Infer();
        ...
        decodeOutputs(Detecter::net_pred_, objects, scale, img_w, img_h);
    ...
    void  Detecter::initalize_infer()
    {
        ...
        Detecter::infer_request_=infer_request;
        ...
        Detecter::mblob_=mblob;
    }
    Detecter::~Detecter()
{
}
InferenceEngine::InferRequest Detecter::infer_request_;
InferenceEngine::MemoryBlob::Ptr Detecter::mblob_;
const float* Detecter::net_pred_;
cv_bridge::CvImagePtr Detecter::cv_image_;
sensor_msgs::CameraInfoConstPtr Detecter::info_;
